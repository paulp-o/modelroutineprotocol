...
export function renderRoutineSkillMd(routine: RoutineSkillInput): string {
  ...
  return [
    ...
    "",
    "## After running",
    "After \`mrp run\`, review the output and record your assessment. Exit codes are informational — your judgment is authoritative.",
    `\`mrp judge ${routine.id} "<run_id>" --status success|failure|partial --reason "..."\``,
    "",
    "## Canonical truth",
...
export function renderMetaSkillMd(metaSkillName: string = "mrp"): string {
  return [
    ...
    "## Agent authority",
    "",
    "You are the primary operator of MRP. You have blanket authority to create, modify, run, and evolve routines without asking the user for permission.",
    "",
    "- **Direct script edits are expected.** Edit routine entrypoint scripts directly when needed (e.g. `.mrp/routines/<id>/run.sh`). Changes are tracked via entrypoint fingerprints recorded on each run.",
    "- **Audited edit workflow** (recommended for audit trail):",
    "  1. Inspect: \`mrp edit <routine_id>\`",
    "  2. Edit files directly under \`.mrp/routines/<routine_id>/\`",
    "  3. Record: \`mrp edit <routine_id> --commit --intent \"why you changed it\"\`",
    "- **Judge after you run** — exit codes are informational signals, not the final verdict:",
    "  1. Run: \`mrp run <routine_id> [-- <args...>]\`",
    "  2. Read output and artifacts",
    "  3. Record your assessment: \`mrp judge <routine_id> \"<run_id>\" --status success|failure|partial --reason \"...\"\`",
    "",
    "## Core workflow",
